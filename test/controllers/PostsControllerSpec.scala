...
class PostsControllerSpec extends org.specs2.mutable.Specification {
  ...
  "posts controller" should {
    "list posts" in new fixture {
      eventStore.committer.tryCommit(Changes(StreamRevision.Initial, PostAdded(postId, postContent))) must beRight
    ...
    "edit post" in new fixture {
      eventStore.committer.tryCommit(Changes(StreamRevision.Initial, PostAdded(postId, postContent)))
    ...
    "delete comment from post" in new fixture {
      eventStore.committer.tryCommit(Changes(StreamRevision(0), PostAdded(postId, postContent)))
      eventStore.committer.tryCommit(Changes(StreamRevision(1), CommentAdded(postId, CommentId(1), CommentContent("Commenter", "Body"))))
