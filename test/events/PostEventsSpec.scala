  ...
  "Post comment event" should {
    ...
    "conflict with post comment with same id" in {
      conflictsWith.conflicting(
        Seq(commentAdded),
        Seq(commentAdded)) must_== Seq(commentAdded)
    }
    ...
    "conflict with other post events" in {
      conflictsWith.conflicting(
        Seq(PostDeleted(postId)),
        Seq(PostEdited(postId, PostContent(author = "Author", title = "Title", body = "Body")))) must_== Seq(PostDeleted(postId))
    }
  }
}
