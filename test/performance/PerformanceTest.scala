...
//
//  val connMgr = new ThreadSafeClientConnManager(new DefaultedHttpParams, )
...
//
//  {
//    val register = new HttpPost(randomHost(random) + routes.UsersController.registerSubmit)
...
//    execute(register)
//
//    val login = new HttpPost(randomHost(random) + routes.UsersController.authenticationSubmit)
