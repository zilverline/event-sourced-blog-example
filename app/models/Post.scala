 ...
 */
case class Posts(byId: Map[PostId, Post] = Map.empty, orderedByTime: Seq[PostId] = Vector.empty) extends (PostId => Post) {
  def apply(id: PostId) = byId(id)
  def get(id: PostId) = byId.get(id)
  def mostRecent(n: Int) = orderedByTime.takeRight(n).reverse.map(byId)

  def apply(event: PostEvent) = event match {
    case PostAdded(id, content)  => copy(byId = byId.updated(id, Post(id, content)), orderedByTime = orderedByTime :+ id)
    case PostEdited(id, content) => copy(byId = byId.updated(id, byId(id).copy(content = content)))
    case PostDeleted(id)         => copy(byId = byId - id, orderedByTime = orderedByTime.filterNot(_ == id))
  }
}
object Posts {
