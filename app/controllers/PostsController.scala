...
class PostsController(actions: ApplicationActions[Posts, PostEvent]) {
  ...
  def show(id: PostId) = QueryAction { posts => implicit request =>
    posts.get(id) map { post =>
      Ok(views.html.posts.show(post, commentContentForm))
    } getOrElse {
      notFound
    }
  }
   ...
   */
  def showAdd = AuthenticatedQueryAction { user => _ => implicit request =>
    if (user.canAddPost) {
  ...
  def add(id: PostId) = AuthenticatedCommandAction { user => _ => implicit request =>
    if (user.canAddPost) {
   ...
   */
  def showEdit(id: PostId) = AuthenticatedQueryAction { user => posts => implicit request =>
  ...
  def edit(id: PostId, expected: StreamRevision) = AuthenticatedCommandAction { user => posts => implicit request =>
    posts.get(id).filter(user.canEditPost) map { post =>
      postContentForm.bindFromRequest.fold(
        formWithErrors =>
   ...
   */
  private def commentContentForm(implicit context: CurrentUserContext): Form[CommentContent] = Form(
  ...
  def addComment(postId: PostId, expected: StreamRevision) = CommandAction { posts => implicit request =>
  ...
  def deleteComment(postId: PostId, expected: StreamRevision, commentId: CommentId) = AuthenticatedCommandAction { user => posts => implicit request =>
    (for {
